...
import { WhereOptions, Includeable, Transaction, Order } from 'sequelize';
import { ValidationError, ForbiddenError } from '@/common/utils/errors.util';
import {
  Taxonomy,
  TemplateType,
  TemplateStatus,
...
import { OptionKeys, TemplateMetaKeys } from '../utils/keys.util';
import {
  TemplateModel,
  TemplateOptionArgs,
  TemplateOptionModel,
...
export class TemplateDataSource extends MetaDataSource<TemplateMetaModel, NewTemplateMetaInput> {
  ...
  private async storeTrashStatus(templateId: number, prevStatus: TemplateStatus, t?: Transaction) {
    return await this.models.TemplateMeta.bulkCreate(
      [
        {
          templateId,
          metaKey: TemplateMetaKeys.TrashStatus,
          metaValue: prevStatus,
        },
        {
          templateId,
          metaKey: TemplateMetaKeys.TrashTime,
  ...
  private async deleteStorageTrashStatus(templateId: number | number[], t?: Transaction) {
    return await this.models.TemplateMeta.destroy({
      where: {
        templateId,
        metaKey: [TemplateMetaKeys.TrashStatus, TemplateMetaKeys.TrashTime],
      },
      transaction: t,
    });
  }
    ...
    for (const { taxonomyId, taxonomyName, taxonomyType } of taxonomies) {
      if ((taxonomyId || taxonomyName) && !taxonomyType) {
        throw new ValidationError(
          await this.i18nService.tv('template.datasource.taxonomy_type_is_required', 'Taxonomy type is required!'),
        );
      }
      ...
      if (taxonomyId) {
        ...
        if (
          ...
          (await this.getOption(OptionKeys.DefaultCategory)) === String(taxonomyId)
        ) {
          andWhere.push({
            [`$TermRelationships.TermTaxonomy.${this.field('id', this.models.TermTaxonomy)}$`]: {
              [this.Op.or]: [taxonomyId, { [this.Op.is]: null }],
            },
          });
        } else {
          andWhere.push({
  ...
  async getCountByStatus(type: string, requestUser: RequestUser) {
    ...
    const where: WhereOptions<TemplateAttributes> = {
      ...
      status: {
        // 不包含所有的操作时的状态
        [this.Op.notIn]: [...Object.values(TemplateOperateStatus)],
      },
      [this.Op.and]: andWhere,
    };
  ...
  async getPaged(
    ...
    if (!requestUser) {
      ...
      if (query.status) {
        ...
        andWhere.push({
          status: {
            [this.Op.notIn]: [TemplateStatus.Trash],
          },
        });
      }
    }
  ...
  async updateName(id: number, name: string, requestUser: RequestUser): Promise<boolean> {
    ...
    if (!!name) {
      throw new ForbiddenError(
        await this.i18nService.tv('datasource.template.name_is_invalid', `Name is invalid!`, {
          lang: requestUser.lang,
        }),
      );
    }
