...
import { Op, ModelDefined, ModelStatic, ProjectionAlias } from 'sequelize';
import { I18nService } from 'nestjs-i18n';
import { ForbiddenError } from 'nestjs-identity';
...
export abstract class BaseDataSource implements OnModuleInit {
  private __AUTOLOAD_OPTIONS__: Dictionary<string> = {}; // Autoload options 缓存
  private __OPTIONS__: Dictionary<string> = {}; // Not autoload options 缓存
  ...
  protected get autoloadOptions() {
    ...
    if (!isEmpty(this.__AUTOLOAD_OPTIONS__)) {
      ...
      return (async () => {
        ...
        const options: Dictionary<string> = await this.models.Options.findAll({
  ...
  protected get tablePrefix() {
    return this.configService.get<string>('database.tablePrefix', '');
  }

  /**
   * 获取配置
   */
  protected getConfig<R = any>(key: string, defaultValue: R): R {
    return this.configService.get<R>(key, defaultValue);
  }
