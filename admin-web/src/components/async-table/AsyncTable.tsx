import { defineComponent, reactive, computed, watch, inject } from '@vue/composition-api';
...
export interface Column extends AntColumnProps {
  /**
   * 是否需要汇总
   */
  needTotal?: boolean;
...
export interface AsyncTableProps extends AntTableProps {
...
export default defineComponent({
  ...
  props: {
    ...
    alert: { type: [Object, Boolean] as PropType<AsyncTableProps['alert']> },
    i18nKeyPrefix: { type: String, default: 'components.async_table' },
  },
  setup(props, { attrs, slots, listeners, refs }) {
    ...
    const data = reactive({
      localLoading: false,
      localDataSource: [] as Record<string, any>[],
      localPagination: false as false | OmitVue<PaginationConfig>, // create 里重新计算
      needTotalList: [] as Array<{
    ...
    const showAlert = computed(() => {
      if (props.alert === void 0) return false;
    ...
    const loadData = (
      ...
      pagination &&
        typeof data.localPagination === 'object' &&
        (data.localPagination = Object.assign({}, data.localDataSource, pagination));
      const params = Object.assign(
        {
          page:
            pagination?.current ||
            (props.showPagination && typeof data.localPagination === 'object' && data.localPagination.current) ||
            props.pageNum,
          size:
            pagination?.pageSize ||
            (props.showPagination && typeof data.localPagination === 'object' && data.localPagination.pageSize) ||
            props.pageSize,
        },
      ...
      return promisify(result)
        .then((r) => {
          ...
          if (
            ...
            typeof data.localPagination === 'object' &&
            data.localPagination.current! > 1
          ) {
          ...
          data.localDataSource = rows; // 返回结果中的数组数据

          // 更新选中记录
          // 1，初始化设置了selectedRowKeys, 要更新 selectedRows
          // 2, 表格数据删减后，选中项要删除
          if (data.selectedRowKeys.length) {
            data.selectedRows = rows.filter((record: Record<string, any>, i: number) =>
              rows.includes(record[getRowKey(record, i)]),
            );
            data.selectedRowKeys = data.selectedRows.map(
    ...
    const refresh = (force = false) => {
      force && (data.localPagination = Object.assign({}, data.localPagination, { current: 1 }));
      loadData();
    };
    ...
    const updateSelect = (selectedRowKeys: Array<string | number>, selectedRows: object[]) => {
      ...
      data.needTotalList = data.needTotalList.map((item) => {
        return {
          ...item,
    ...
    watch([() => route.params, () => route.query], ([params, query]) => {
      if (router && data.localPagination && props.pageURI) {
        ...
        let changed = false;
        if (!Number.isNaN(page) && page !== data.localPagination.current) {
          data.localPagination.current = page;
          changed = true;
        }

        if (!Number.isNaN(size) && size !== data.localPagination.pageSize) {
          data.localPagination.pageSize = size;
          changed = true;
        }
    ...
    watch(
      () => data.localPagination,
      (pagination) => {
        if (router && pagination && props.pageURI) {
          // query 直接修改原始值会报重复路由
          ...
          const { resolved, href: nextRoute } = router.resolve({ ...route, name: route.name!, params, query });
          resolved.fullPath !== route.fullPath && router.push(nextRoute);
        }
      },
    );

    data.needTotalList = initTotalList(props.columns);
    data.selectedRowKeys = (attrs.rowSelection as any)?.selectedRowKeys || [];
    // 初始化加载数据
    loadData();
    ...
    return () => {
      const renderAlert = () => {
        // 绘制统计列数据
        const needTotalItems = data.needTotalList.map((item) => {
          return (
              ...
              <span style="margin-right: 12px">
                ...
                <span style="font-weight: 600">
                  {configProvider.i18nRender(
                    ...
                    `${data.selectedRows.length} Row(s)`,
                    {
        ...
        function renderClear(callback: () => void) {
          if (data.selectedRowKeys.length <= 0) return null;
          return (
            <a
