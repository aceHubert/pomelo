...
import { dbLockFileName } from '../db.sync';
import { InitArgs } from './interfaces/init-args.interface';

@Injectable()
export class DataInitService {
  private readonly logger = new Logger(DataInitService.name, { timestamp: true });
  private readonly dbLockFile = new LockFile(dbLockFileName);
  ...
  hasDatasInitialed() {
    return this.dbLockFile.getEnvValue('INIT_DATAS_REQUIRED') !== 'false';
  }

  /**
   * Initialize the datas
   */
  async initDatas(initArgs: InitArgs) {
    const needInitDatas = this.hasDatasInitialed();
    if (!needInitDatas) {
      this.logger.log('Datas have been already initialized!');
      return;
    }
    ...
    this.dbLockFile.setEnvValue('INIT_DATAS_REQUIRED', 'false');
