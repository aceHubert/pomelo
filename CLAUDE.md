...
- **Frontend**: Vue.js 2, TypeScript, Ant Design Vue V1.x, Vant, Formily
...
This project uses CSS Modules for component-scoped styling to avoid class name conflicts and provide better maintainability.

### File Naming Convention
- CSS Module files use the `.module.less` extension
- Located alongside their corresponding components
- Example: `login/views/login/index.vue` uses `login/views/login/index.module.less`
...
<style module>
/* Alternatively, use <style module> for single-file components */
.container {
  /* styles here */
}
</style>
...
- Use kebab-case or camelCase for class names
- Access classes via `$style.className` in templates
- Use nested selectors for better organization:
```less
.container {
  padding: 20px;
  ...
  :global {
    /* Use :global() to target global CSS classes */
    .ant-form-item {
      margin-bottom: 16px;
    }
  }
...
### Global Styles
- Use `:global()` wrapper to target global CSS classes
- Useful for styling third-party components (Ant Design Vue, etc.)
- Example: `:global(.ant-btn-primary) { ... }`

### Responsive Design
- Use media queries within module files
...
- Nesting: supported
- Mixins: can be imported from shared files
- Functions: color manipulation, math operations, etc.
