...
import { OptionKeys } from '@/common/utils/option-keys.uti';
...
export class TemplateDataSource extends MetaDataSource<TemplateMetaModel, NewTemplateMetaInput> {
  ...
  private async storeTrashStatus(templateId: number, prevStatus: TemplateStatus, t?: Transaction) {
    return await this.models.TemplateMeta.bulkCreate(
      [
        {
          templateId,
          metaKey: TRASH_META_STATUS_KEY,
          metaValue: prevStatus,
        },
        {
          templateId,
          metaKey: TRASH_META_TIME_KEY,
  ...
  private async deleteStorageTrashStatus(templateId: number | number[], t?: Transaction) {
    return await this.models.TemplateMeta.destroy({
      where: {
        templateId,
        metaKey: [TRASH_META_STATUS_KEY, TRASH_META_TIME_KEY],
      },
      transaction: t,
    });
  }
