...
export interface EnvConfig {
  basicApiBase: string;
  basicGraphqlBase: string;
  identityGraphqlBase: string;
  graphqlSubscriptionBase?: string;
  getOidcSettings: (baseUrl?: string) => UserManagerSettings;
}
...
export const envConfig: EnvConfig = {
  ...
  getOidcSettings: (baseUrl = '/') => ({
    authority: origin + baseUrl + 'identity/oauth2',
    client_id: clientId,
